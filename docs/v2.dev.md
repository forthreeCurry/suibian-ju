...
## 附录 A：Python API（FastAPI）环境与模块

运行目录：`api/`。依赖见 `api/requirements.txt`（含 `httpx`、`supabase`）。

| 环境变量 | 说明 |
|---------|------|
| `GAODE_API_KEY` | 高德 Web 服务 Key（POI / 逆地理 / IP 等） |
| `GAODE_QPS_MAX` | 可选，客户端 QPS 上限，默认 `50` |
| `SUPABASE_URL` | Supabase 项目 URL |
| `SUPABASE_KEY` | 服务端/anon Key（同步与列表接口需写权限时用 service_role 或具备 insert 的 key） |

相关代码：

- `api/app/services/gaode_service.py` — `GaodeService`（`place/text`、`place/around`、`geocode/regeo`、`ip`）
- `api/app/services/data_sync.py` — `DataSyncService`（商圈同步、`BEIJING_HOT_AREAS`）
- `api/app/models/restaurant.py` — `GaodeRestaurant`、`Restaurant`、`RestaurantCreate`

临时调试路由（`api/app/main.py`）：

- `POST /api/v2/sync/area` — body：`area_name`, `lat`, `lng`, `radius`
- `POST /api/v2/sync/popular` — 同步 10 个预定义北京商圈
- `GET /api/v2/restaurants` — 查询已入库餐厅：`page`, `page_size`, `cuisine`
